...
def simplify_complex_num(number:str) -> str:
    ...
    if (len(numberparts) > 1):
        ...
        return number
'''
Given a test-case filename it will read the contents and store test-cases in appropriate dictionaries
Only loads test-cases from the tests_used folder
args: filename = str
returns: tests = dict
'''
...
'''
This section executes first, and calls the load_tests function for every test-case file
'''
cwd = os.getcwd()
print(cwd)
...
'''
This PyTest function collects the test-cases from the tests dictionary
and prepares each testing function with its inputs
'''
...
'''
This testing function determines if the discriminant MR is true for both the seed problem and it's transformation
PyTest checks the assert statement for test pass/failure
args: receives input tests dictionary from PyTest collection
'''
...
'''
This testing function determines if the vieta equations MR is true for both the seed problem and it's transformation
PyTest checks the assert statement for test pass/failure
args: receives input tests dictionary from PyTest collection
'''
...
'''
This testing function determines if the equation evaluation MR is true for both the seed problem and it's transformation
PyTest checks the assert statement for test pass/failure
args: receives input tests dictionary from PyTest collection
'''
...
'''
This function checks if the discriminant MR property holds for a given problem
discriminant metamorphic relation tests
d = b**2-4ac
d > 0 => 2 roots
d = 0 => 1 root
d < 0 => 2 complex roots
returns true if the MR holds, false otherwise
'''
